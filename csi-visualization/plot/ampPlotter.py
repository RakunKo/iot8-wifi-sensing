...
def lowpass_hampel(signal, thresh=0.63, wavelet="db4", window_size=5, k=3):
    # Lowpass filter using DWT
    denoised_signal = lowpassfilter(signal, thresh, wavelet)
    # Hampel filter
    hampel_filtered = medfilt(denoised_signal, kernel_size=window_size)
    return hampel_filtered

def AmpPlotter(csi_df, sample_start, sample_end, fname, spf_sub_list=None, thresh=0.63, wavelet="db4", hampel_window=5, hampel_k=3):
    ...
    if spf_sub_list is not None:
        ...
        # ============ Denoising with Lowpass and Hampel Filters ==================

        fig, ax = plt.subplots(figsize=(12, 8))
        fig.suptitle('Amp-PacketIdx plot', fontsize=20)

        for sub_csi in sub_csi_list:
            signal = np.array(sub_csi[1])

            # Lowpass and Hampel filter
            filtered_signal = lowpass_hampel(signal, thresh=thresh, wavelet=wavelet, window_size=hampel_window, k=hampel_k)

            ax.plot(filtered_signal, alpha=0.5, label='sub ' + sub_csi[0])
        ...
        for idx, sub in enumerate(subcarrier_list):
            ...
            ax.plot(filtered_signal, alpha=0.8, label=csi_df.columns[idx])
